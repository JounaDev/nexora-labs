...
export default async function ReparacionDetallePage({ params }: Props) {
  ...
  // Evita que un cliente pueda acceder al ticket de otro
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <GlassCard>
          ...
          {ticket.warranty ? (
            ...
            <p className="text-sm text-text-muted">
              Se activa al finalizar la reparación.
            </p>
          )}
        </GlassCard>
...
<GlassCard>
  ...
  <div className="space-y-2 text-sm">
    <div className="flex justify-between">
      ...
      <span>{currency.format(Number(ticket.laborCost ?? 0))}</span>
    </div>

    <div className="flex justify-between">
      <span className="text-text-muted">Repuestos</span>
      <span>{currency.format(Number(ticket.partsCost ?? 0))}</span>
    </div>

    <div className="flex justify-between">
      <span className="text-text-muted">Impuestos</span>
      <span>{currency.format(Number(ticket.tax ?? 0))}</span>
    </div>

    <div className="flex justify-between">
      <span className="text-text-muted">Descuento</span>
      <span>-{currency.format(Number(ticket.discount ?? 0))}</span>
    </div>

    <div className="mt-3 border-t border-border/10 pt-3 flex justify-between font-bold">
      <span>Total</span>
      <span>{currency.format(Number(ticket.finalCost ?? 0))}</span>
    </div>
  </div>
</GlassCard>
          {ticket.invoice ? (
            <p className="text-sm text-text-muted">
              ...
              <span className="text-text">
                {currency.format(Number(ticket.invoice.total))}
              </span>{" "}
